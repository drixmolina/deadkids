...
function LoadingScreen() {
  ...
  if (!show) return null;
  return <div className="loader-screen"><div className="loader-mark">DDKDS</div><p>PRIVATE DROP LOADING...</p></div>;
}
...
function AdminMessages(){const [rows,setRows]=useState([]); const load=()=>request('/api/contact').then(setRows); useEffect(load,[]); const mark=async(m,status)=>{await request(`/api/contact/${m.id}`,{method:'PUT',body:JSON.stringify({status})});load()}; const del=async id=>{if(confirm('Delete this message?')){await request(`/api/contact/${id}`,{method:'DELETE'});load()}}; return <><h1>Contact Messages</h1><div className="grid two">{rows.map(m=><div className="card" key={m.id}><h3>{m.name}</h3><p>{m.email}</p><p>{m.message}</p><span className="badge">{m.status}</span><div className="row-actions"><button onClick={()=>mark(m,m.status==='Unread'?'Read':'Unread')}>Mark {m.status==='Unread'?'Read':'Unread'}</button><button className="danger-btn" onClick={()=>del(m.id)}>Delete</button></div></div>)}</div></>}
...
function AdminSettings(){
  ...
  useEffect(()=>{request('/api/site-content').then(x=>setF({...defaults,...x})).catch(()=>{})},[]);
  const save=async e=>{e.preventDefault(); const payload={...f,music_video_id:extractYouTubeId(f.music_video_id)}; await request('/api/site-content',{method:'PUT',body:JSON.stringify(payload)}); setF(payload); alert('Website settings saved!')};
